...
def escutar_eventos(contract, web3, server_account, private_key):
    ...
    # Cria o filtro a partir do bloco atual
    ultimo_bloco = web3.eth.block_number
    logger.info(f"Criando filtro a partir do bloco {ultimo_bloco}")
    event_filter = contract.events.NovoBloco.create_filter(from_block=ultimo_bloco)
    logger.info("Filtro de eventos criado com sucesso!")

    # Obtém o ID da instância atual (pode ser baseado no endereço da conta)
    instancia_id = int(server_account.address, 16) % 1000  # Gera um ID único para esta instância
    logger.info(f"""
    ===== Configuração da Instância =====
    Endereço da conta: {server_account.address}
    ID da instância: {instancia_id}
    Número da instância: {(instancia_id % 3) + 1}
    ===================================
    """)
    ...
    while True:
        try:
            ...
            # Tenta obter eventos
            eventos = event_filter.get_new_entries()
            if eventos:
                ...
                for event in eventos:
                    ...
                    # Obtém informações do bloco atual
                    bloco_info = web3.eth.get_block(bloco_atual)
                    
                    # Mecanismo de eleição: usa o hash do bloco para determinar qual instância processa
                    bloco_hash = int(bloco_info['hash'].hex(), 16)
                    eleito = (bloco_hash % 3) + 1  # Gera um número entre 1 e 3
                    
                    logger.info(f"""
                    ===== Eleição de Processamento =====
                    ID do Bloco: {bloco_id}
                    Hash do Bloco: {bloco_info['hash'].hex()}
                    Hash numérico: {bloco_hash}
                    Instância Eleita: {eleito}
                    ID desta Instância: {instancia_id}
                    Número desta Instância: {(instancia_id % 3) + 1}
                    Esta instância foi eleita? {eleito == (instancia_id % 3) + 1}
                    =================================
                    """)

                    # Verifica se esta instância foi eleita para processar
                    if eleito == (instancia_id % 3) + 1:
                        logger.info("✅ Esta instância foi eleita para processar o bloco!")
                        # Busca bloco completo
                        bloco = contract.functions.getBloco(bloco_id).call()
                    else:
                        logger.info("❌ Esta instância não foi eleita para processar o bloco. Aguardando próximo bloco...")
                        continue
                    ...
                    while True:
                        ...
                        distancia_restante = calcular_distancia(x_atual, y_atual, bloco[3], bloco[4])
                        ...
                        x_parada, y_parada = calcular_ponto_parada(x_atual, y_atual, bloco[3], bloco[4], bateria_atual)
...
if __name__ == '__main__':
    try:
        logger.info("Iniciando Central 3...")
        ...
        logger.info(f"""
        ============================================
        Servidor Central 3 Iniciado com Sucesso!
